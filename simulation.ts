class simulation 
{
    ...
    canvasGrid: grid;
    snakeCoordinates: Point[]=[];
    fruitCoordinate: Point;
    currentKeyCode:number = 39;
    isHorizontalMotion:boolean = true;
    nextKeyCode:number;
    start:number = -Math.PI/2;
    end:number = Math.PI/2;
    backward;
    animationIsOn: boolean = true;
    ...
    random():Point
    {
        ...
        Math.ceil
        var pt:Point = new Point( Math.ceil(x), Math.ceil(y))
    ...
    computeNextCoordinate(keyCode: any) {
    ...
    compare(pt1:Point,pt2:Point):boolean
    {
